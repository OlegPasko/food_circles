...
class UserMailer < ActionMailer::Base
  ...
  def company_signup(email, name, _company)
    mail(to: email, subject: 'Thanks for signing up.', body: "Someone will get back with you soon, #{name}.")
  end
  ...
  def organizers_notify(email, location, _address, date, num_diners, occassion, budget, food_preferences, donation, feedback)
  ...
  def monthly_invoice(venue, months_before = 1)
    ...
    mail(to: venue.email, cc: ADMIN_EMAIL, reply_to: 'jk@joinfoodcircles.org', subject: 'FoodCircles in hiberation in Feb. Jan impact report enclosed.')
  end
  ...
  def badge_email(_email, category, title)
    @category = category
    @title = title
    mail(
