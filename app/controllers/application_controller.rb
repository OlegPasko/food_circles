...
class ApplicationController < ActionController::Base
  ...
  def check_subdomain
    unless ['', 'www', 'staging', 'testing'].include?(request.subdomain)
      ...
      redirect_to BASE_URL unless sub_charity
    end
  end
  ...
  def prepare_for_mobile
    ...
    if mobile_device? && request.fullpath == '/'
      if android_device?
  ...
  def detect_email_omniauth
    unless current_user.nil?
      if current_user.email.blank? && controller_name != 'omniauth_ask_for_email'
