...
class Api::TimelineController < ApplicationController
  ...
  def show
    ...
    hash = { weekly_total: @weekly_total,
             ...
             payments: @payments.map do |p|
                         { id: p.id,
                           ...
                           offer: [Offer.find(p.offer_id)].map do |o|
                                    { id: o.id,
                                      name: o.name,
                                      details: o.details,
                                      ...
                                      venue: [Venue.find(o.venue_id)].map do |v|
                                               { id: v.id,
                                                 name: v.name,
             ...
             reservations: @reservations.map do |r|
                             ...
                             data[:offer] = if r.offer.present?
                                              r.offer.as_json.slice(:id, :title, :details, :minimum_diners)
                                               .merge(discount_price: r.offer.price,
                             ...
                             data[:venue] = if r.venue.present?
                                              r.venue.as_json.slice(:id, :name, :city, :state, :zip, :lat, :lon, :description, :phone, :web, :tags, :offers).merge(open_times: r.venue.times || 'Not Available',
  ...
  def use_voucher
    render(json: { error: true, description: "There's currently no way to use this, so please tell me how should I implement it." }, status: 503) && return
  end

  def verify_payment_and_show_voucher
    # TO BE IMPLEMENTED BY THE MOBILE APP DEVELOPER
    render(json: { error: true, description: 'To be implemented by the mobile app developer.' }, status: 503) && return
  end
end
