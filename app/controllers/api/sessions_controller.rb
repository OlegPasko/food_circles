...
class Api::SessionsController < ApplicationController
  ...
  def update_profile
    ...
    if @user.update_attributes(params[:session])
      ...
      if !@user.errors.messages.empty?
        render(json: { error: true, description: 'Error saving user.', errors: @user.errors.messages }, status: 500) && return
      else
        render(json: { error: true, description: 'Error saving user.' }, status: 500) && return
      end
    end
  rescue Exception => e
  ...
  def password_email_sign_in
    ...
    if @user.valid_password?(params[:user_password])
      ...
      render(json: { error: true, description: 'Wrong password.' }, status: 401) && return
    end
    render json: { error: false, auth_token: @auth_code.to_s }
  rescue Exception => e
    render(json: { error: true, description: 'Internal Server Error.' }, status: 503) && (return)
  end
  ...
  def social_sign_in
    if external_uid = ExternalUID.where(uid: params[:uid]).first
      ...
      render json: { error: false, description: 'User retrieved.', auth_token: @auth_code.to_s }
    else
